...
author: "Last updated at `r paste0(format(Sys.time(), '%H:%M'), ' on ', format(Sys.time(), '%A %d %B %Y'))`"
output: 
...
library(flexdashboard) ; library(knitr) ; library(tidyverse) ; library(rtweet) ; library(tidytext) ; library(reshape2) ; library(igraph) ; library(grid) ; library(ggraph) ; library(lubridate) ; library(DT) ; library(htmltools)
...
# get tweets
tweets <- search_tweets(q = '@TraffordCouncil OR "Trafford Council"',
                          n = 18000,
                          include_rts = FALSE,
                          `-filter` = "replies",
...
tweets %>% 
  ...
  datatable(class = 'cell-border stripe',
            ...
            em("Tweets mentioning @TraffordCouncil or 'Trafford Council'")),
            width = "100%",
            options = list(
...
tweets %>% 
  ...
  labs(x = NULL, y = NULL,
       ...
       subtitle = paste0("@TraffordCouncil  or 'Trafford Council' mentions, ", format(min(tweets$created_at), "%d-%m"), " to ", format(max(tweets$created_at),"%d-%m-%Y"))) +
  coord_flip() +
  guides(fill = FALSE) +
  theme_x()
```
