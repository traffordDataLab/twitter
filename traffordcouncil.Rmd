---
title: "@TraffordCouncil mentions"
output: 
...
# get tweets
tweets <- search_tweets(q = paste0(handle, ' OR "Trafford Council"'),
                          n = 18000,
                          include_rts = FALSE,
                          `-filter` = "replies",
...
This dashboard retrieves tweets over the last 6-9 days that mention Trafford Council. Tweets were last retrived at *`r paste0(format(Sys.time(), '%H:%M'), ' on ', format(Sys.time(), '%d %B %Y'))`* using the following search parameters: '@TraffordCouncil' OR "Trafford Council"'. The [rwteet](https://cran.r-project.org/web/packages/rwteet) [R](https://www.r-project.org/) package was used to retrieve the tweets and [tidytext](https://cran.r-project.org/web/packages/tidytext) to tokenise them for analysis.   
