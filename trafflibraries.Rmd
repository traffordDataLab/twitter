...
This dashboard retrieves tweets over the last 6-9 days that mention Trafford's libraries. Tweets were last retrived at *`r paste0(format(Sys.time(), '%H:%M'), ' on ', format(Sys.time(), '%d %B %Y'))`* using the following search parameters: '@TraffLibraries'  OR "Altrincham library" OR "Coppice library" OR "Hale library" OR "Trafford Local Studies" OR "Lostock library" OR "Old Trafford library" OR "Partington library" OR "Sale library" OR "Stretford library" OR "Timperley library" OR "Trafford Town Hall Library" OR "Urmston library" OR "Woodsend library"'. The [rwteet](https://cran.r-project.org/web/packages/rwteet) [R](https://www.r-project.org/) package was used to retrieve the tweets and [tidytext](https://cran.r-project.org/web/packages/tidytext) to tokenise them for analysis.   
